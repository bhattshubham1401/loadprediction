...
#         end_date = "2023-11-18"
#         url = f"https://archive-api.open-meteo.com/v1/archive?latitude=28.58&longitude=77.33&start_date={start_date}&end_date={end_date}&hourly=temperature_2m,relative_humidity_2m,rain,cloud_cover,precipitation,wind_speed_10m&timezone=auto"
...
#                         "precipitation":data_dict['precipitation'][i],
#                         })
...
def initialize_mongodb(collection_name):
    try:
        ...
        collection_name5 = os.getenv("collection5")
        collection_name6 = os.getenv("collection6")
        collection_name7 = os.getenv("collection7")
        ...
        collection5 = db1[collection_name5]
        collection6 = db1[collection_name6]
        collection7 = db1[collection_name7]

        if type(collection_name)== list:
            return client, collection5, collection6, collection7
        elif collection_name == "forecast1":
            return client, collection1
        elif collection_name == "forecast2":
            return client, collection2
        elif collection_name == "forecast3":
            return client, collection3
        elif collection_name == "forecast4":
            return client, collection4
        elif collection_name == "sensor":
            return client, collection5
        elif collection_name == "train":
            return client, collection6
        elif collection_name == "test":
            return client, collection7
    except Exception as e:
            print(e)  

@ensure_annotations
def store_sensor_data_in_db(collection1, collection2, collection3, sensor_id,\
    ...
    try:
        if sensor_id == 0 :
            ...
            collection2.drop()
            collection3.drop()
            logger.info("all collection dropped")
...
@ensure_annotations
def collection_deletion(collection):
    try:
        collection.drop()
