...
from src.mlProject.utils.common import create_features, add_lags, store_predictions_in_mongodb,initialize_mongodb, data_fetching
...
class PredictionPipeline:
    ...
    def predict(self):
        try:
            ...
            # # Concatenate data for all sensors
            # train_data = pd.concat(data_list, ignore_index=True)
            client, collection = initialize_mongodb("predict")
            ...
            for i in range(count):
                test_data, name = data_fetching("foracast3")
                ...
                future = pd.date_range(start='2023-11-01',end='2023-12-31', freq='1H')
                future_df = pd.DataFrame(index=future)
                future_df['isFuture'] = True
                df1['isFuture'] = False
                df_and_future = pd.concat([df1, future_df])
